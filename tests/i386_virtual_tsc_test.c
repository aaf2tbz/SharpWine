...
static const uint8_t sequence[] = {0x90U, 0x0fU, 0x31U, 0x90U, 0x66U, 0x0fU, 0x31U};
...
static void exercise(enum gem_i386_engine_mode mode) {
    ...
    assert(gem_i386_runtime_run(whole, &whole_context, 4U) == GEM_STOP_HOST_RETURN);
    assert(gem_i386_runtime_last_stop_info(whole, &stop));
    assert(stop.instructions_retired == 4U);
    assert(whole->virtual_tsc == 4U);
    assert(whole_context.gpr[GEM_I386_EAX] == 3U);
    assert(whole_context.gpr[GEM_I386_EDX] == 0U);

    assert(gem_i386_runtime_run(split, &split_context, 2U) == GEM_STOP_BUDGET_EXPIRED);
    assert(gem_i386_runtime_last_stop_info(split, &stop));
    assert(stop.instructions_retired == 2U);
    assert(split->virtual_tsc == 2U);
    assert(split_context.gpr[GEM_I386_EAX] == 1U);
    assert(split_context.gpr[GEM_I386_EDX] == 0U);
    assert(gem_i386_runtime_run(split, &split_context, 2U) == GEM_STOP_HOST_RETURN);
    assert(gem_i386_runtime_last_stop_info(split, &stop));
    assert(stop.instructions_retired == 2U);
    assert(split->virtual_tsc == 4U);
